...
export default function MapPage() {
  ...
  const switchMapType = () =>
    setMapType((oldType) => (oldType === "google" ? "osm" : "google"));
  ...
  return (
    <Content>
      ...
      <Sheet
        width={orientation === LANDSCAPE ? LANDSCAPE_WIDTH : PORTRAIT_WIDTH}
      ...
      >
        <SheetHeader>
          ...
          {orientation === LANDSCAPE && (
            <PrintTitle align="center">{title}</PrintTitle>
          )}
          <Logo />
        </SheetHeader>
        <SheetContent>
          {orientation === PORTRAIT && (
            <PrintTitle align="left">{title}</PrintTitle>
          )}
          <Stacked>
...
const MapGrid = (props) => {
  ...
  const columnNames = Array(numColumns)
    .fill(0)
    .map((_, i) => ALPHABET[i]);
  const rowNames = Array(numRows)
    .fill(0)
    .map((_, i) => i + 1);
  ...
  if (mapType === "google") {
    return (
      <MapElementStyle>
        <GoogleMapReact
          bootstrapURLKeys={{
            key: "AIzaSyC8uc5De4G80Osg0rYWHKCmhbHzfobdvQk&language=de",
            language: "de",
          }}
...
const Sheet = styled.div`
  ...
  grid-template-rows: ${(props) => props.headerPadding} 15mm ${(props) =>
      props.headerPadding} auto 10mm ${(props) => props.headerPadding};
  grid-template-areas:
    ". . ."
    ". header ."
    ". . ."
